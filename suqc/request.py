...
class CoupledDictVariation(VariationBase, ServerRequest):
    METAINFOFILE = "parameter.csv"

    # todo mario:
    #  - temp Verzeichnis schreiben in parent (VariationBase)
    def __init__(
            self,
            ini_path: str,
    ...
    def get_simulations(self):
        ...
        file_path = os.path.join(self.env_man.get_temp_folder(), "parameter.csv")
    ...
    def read_from_temp_folder(self, temp_folder, option="all"):
        ...
        para = os.path.join(temp_folder, "parameter.csv")
        df: pd.DataFrame = pd.read_csv(para)
    ...
    def get_sim_results_from_temp(self):
        ...
        files.remove("env_info.csv")

        path_parameter_file = os.path.join(temp_folder, "parameter.csv")
        if os.path.isfile(path_parameter_file):
            files.remove("parameter.csv")

        # read data and build a dataframe which contains all data

        df = pd.DataFrame()
        for f in files:
            # if os.path.splitext(f)[1] != ".pkl": # todo remore
            if os.path.splitext(f)[1] != ".csv":
                raise ValueError(
                    f"Only .csv files allowed (generated by suqc). Got file: {f}."
                )
    ...
    def write_temp_data(self, par_id, run_id, result, return_code, required_time):
        ...
        temp_file: str = os.path.join(
            self.env_man.get_temp_folder(), f"{par_id}__{run_id}.csv"
        )
        ...
        names: list = df.index.names.copy()
        names[0] = "id"
        names[1] = "run_id"
